...
Imagine a robot sales rep that calls a lead to check if they'd be interested in a CRM. This app
simulates exactly that call, in your browser, with no phone line involved:

1. You add a **lead** (name, phone, company) — just a record in the database.
2. You press **Start AI call**. The agent speaks its opening line out loud, using
   **Text-to-Speech**: real audio generated by OpenAI, not a recording.
3. You answer **by talking into your microphone**. The app sends your recording to OpenAI's
   **Speech-to-Text**, gets back the words you said, and decides what to ask next — team size, how
   they handle leads today, whether they want a demo — based on your actual answer.
4. This repeats until the call naturally ends: either they're not interested, or they've answered
   everything and the call completes.
5. Every single thing said, by both sides, is saved to the database, and a short **summary** plus a
   final **outcome** (qualified, not interested, demo requested, etc.) is generated automatically.

So the "AI" part is two things: OpenAI turning text into speech and speech into text, and a simple
rule-based engine deciding what the agent should ask or say next based on what the customer said.
Nothing here needs a real phone number or a telephony provider — it's a working stand-in for one.

## Screenshots

**Dashboard and leads pipeline** — every lead's status, with counts at the top (new, contacted,
qualified, converted, disqualified):

![Dashboard and leads table](docs/screenshots/dashboard-and-leads.jpg)

**A live call in progress** — the agent's questions on the left in gray, the customer's spoken
answers on the right in green, with a box below showing what's been captured so far:

![Call conversation view](docs/screenshots/call-conversation.jpg)

**Call summary once it finishes** — the outcome, and every answer the agent collected along the
way:

![Call summary and outcome](docs/screenshots/call-summary.jpg)

Everything you need — setup, APIs, configuration, and troubleshooting — is in this one README.
...
71 tests, no network access and no API key required. Integration tests replace the two OpenAI
...
| Database structure | `src/main/java/com/flowzo/callingagent/entity` (`Lead`, `CallRecord`, `ConversationMessage`) |
| API docs | `/swagger-ui.html` once the app is running |
| Sample conversation output | run the [example curl flow](#example-curl-flow) above, or open the browser simulator |
